...
def test_static_planner():
    ...
    # Generate routes using the static planner
    #valid_route = benchmark(route_generator, grid, requests, start_loc)
    valid_route = route_generator(grid, requests, start_loc)
    assert valid_route is not None
    if valid_route:
        print(f'Valid route generated by the static planner: {valid_route}')
        animate_search(grid, requests, start_loc, valid_route)
